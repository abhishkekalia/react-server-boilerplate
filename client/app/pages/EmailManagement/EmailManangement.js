...
import EmailTable from '../../pages/EmailManagement/EmailTable';
...
class EmailManagement extends Component {
  ...
  getallemail() {
    ...
    try {
      ...
      this.props.emaillist(body, access_token, (response) => {
        let { data } = response;
        if (data.status == 1) {
          self.setState({ error: null, alldata: data.data,total:data.total })
          console.log(this.state.total)
          console.log("all  email data ", this.state.alldata)
        }
        else {
      ...
      <Home>
        <div>
          ...
          <div className="animated fadeInUp">
            ...
            <div>
              <EmailFilter show={this.state.show} dataTofilter={this.state.userdata} access_token={this.props.token} applyFilter={this.applyFilter.bind(this)}  cancelFilter={this.cancelFilter.bind(this)}/>
            </div>
            <div>
              <EmailTable columnElement={["email_for","subject"]} sortChange={this.sortChange.bind(this)} userdata={this.state.alldata} Deleteselectedbox={this.Deleteselectedbox.bind(this)} userstatusChange={this.userstatusChange.bind(this)} deleteCms={this.deleteCms.bind(this)} />
...
export default withRouter(connect(mapStateToProps, actions)(EmailManagement));
