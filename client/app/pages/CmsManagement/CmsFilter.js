...
class CmsFilter extends React.Component {
   ...
   //     this.setState({ selectList: [], fetching: true });
        this.props.searchCmsdata(filter, access_token, (response) => {
            let { data } = response;
                                    ...
                                    <div className="form-group row">
                                        <div className="col-3">
                                            ...
                                            <Select
                                                mode="multiple"
                                                labelInValue
                                                value={selectUsers.title}
                                                placeholder="Select users"
                                                notFoundContent={fetching ? <Spin size="small" /> : null}
                                                filterOption={false}
                                                onSearch={this.fetchUser.bind(this,"title")}
                                                onChange={this.handleChange.bind(this)}
                                                style={{ width: '100%' }}
                                            >
                                                {
                                                   ( selectList.title)? (
                                                       selectList.title.map(d => <Option key={d.value}>{d.text}</Option>)
                                                   ):(null) 
                                                }
                                            </Select>
                                        </div>
                                        ...
                                        <div className="col-3">
                                            ...
                                            <Select
                                                 mode="multiple"
                                                labelInValue
                                                value={selectUsers.pageId}
                                                placeholder="Select users"
                                                notFoundContent={fetching ? <Spin size="small" /> : null}
                                                filterOption={false}
                                                onSearch={this.fetchUser.bind(this, "pageId")}
                                                onChange={this.handleChange.bind(this)}
                                                style={{ width: '100%' }}
                                            >
                                            {
                                                  ( selectList.pageId)?(
                                                    selectList.pageId.map(d => <Option key={d.value}>{d.text}</Option>)
                                                  ):(null)
                                            }
