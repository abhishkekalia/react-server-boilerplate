...
class SeoTable extends Component {
  ...
  editEmaillist(dynamicData ) {
    ...
    this.props.submitSeolist(udata, access_token);
    this.props.history.push('/editseo')
  }
              ...
              {this.state.userdata.map((dynamicData, Key) => {
                return (
                  <tr key={dynamicData._id} style={{borderBottom: '1px solid red' }} className="animated fadeIn">
                    ...
                    <td>{dynamicData.pageName} </td>
                    <td>{dynamicData.pageUrl} </td>
                    <td>{dynamicData.metaDescription} </td>
...
export default withRouter(connect(mapStateToProps, actions)(SeoTable));
