...
class SeoFilter extends React.Component {
    constructor(props) {
        super(props);
        this.lastFetchId = 0;
   ...
   //     this.setState({ selectList: [], fetching: true });
        this.props.searchSeodata(filter, access_token, (response) => {
            let { data } = response;
                                    ...
                                    <div className="form-group row">
                                        <div className="col-3">
                                            ...
                                            <Select
                                                mode="multiple"
                                                labelInValue
                                                value={selectUsers.pageName}
                                                placeholder="Select PageName"
                                                notFoundContent={fetching ? <Spin size="small" /> : null}
                                                filterOption={false}
                                                onSearch={this.fetchUser.bind(this,"pageName")}
                                                onChange={this.handleChange.bind(this)}
                                                style={{ width: '100%' }}
                                            >
                                                {
                                                   ( selectList.pageName)? (
                                                       selectList.pageName.map(d => <Option key={d.value}>{d.text}</Option>)
                                                   ):(null) 
                                                }
                                            </Select>
                                        </div>
                                        ...
                                        <div className="col-3">
                                            ...
                                            <Select
                                                mode="multiple"
                                                labelInValue
                                                value={selectUsers.pageUrl}
                                                placeholder="Select PageUrl"
                                                notFoundContent={fetching ? <Spin size="small" /> : null}
                                                filterOption={false}
                                                onSearch={this.fetchUser.bind(this, "pageUrl")}
                                                onChange={this.handleChange.bind(this)}
                                                style={{ width: '100%' }}
                                            >
                                            {
                                                  ( selectList.pageUrl)?(
                                                    selectList.pageUrl.map(d => <Option key={d.value}>{d.text}</Option>)
                                                  ):(null)
                                            }
...
export default withRouter(connect(mapStateToProps, actions)(SeoFilter));